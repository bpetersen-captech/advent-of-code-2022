...
Rock Paper Scissors is a game between two players. Each game contains many rounds; in each round, the players each simultaneously choose 
one of Rock, Paper, or Scissors using a hand shape. Then, a winner for that round is selected: Rock defeats Scissors, Scissors defeats Paper,
...
for a single round is the score for the shape you selected (1 for Rock, 2 for Paper, and 3 for Scissors) plus the score for the outcome of 
...
#first round, opp pick R, you should do P (score=8)
#second round, opp pick P, you should do R (score=1)
#third round, draw with both S (score=6)

score1 = 0 # X for rock, Y for paper, Z for scissors
score2 = 0 # X for lose, Y for draw, Z for win
...
for line in file.readlines():
    if line[0] == '\n':
        ...
        if line[0] == 'A':
            if line[2] == 'X':
                score1 += 4 #rock vs rock (tie)
                score2 += 3 #rock vs scissors (lose)
                continue
            elif line [2] == 'Y':
                score1 += 8 #rock vs paper (win)
                score2 += 4 #rock vs rock (tie)
                continue
            else:
                score1 += 3 #rock vs scissors (lose)
                score2 += 8 #rock vs paper (win)
                continue
        #opp plays paper
        elif line[0] == 'B':
            if line[2] == 'X':
                score1 += 1 #paper vs rock (lose)
                score2 += 1 #paper vs rock (lose)
                continue
            elif line [2] == 'Y':
                score1 += 5 #paper vs paper (tie)
                score2 += 5 #paper vs paper (tie)
                continue
            else:
                score1 += 9 #paper vs scissors (win)
                score2 += 9 #paper vs scissors (win)
                continue
        #opp plays scissors
        elif line[0] == 'C':
            if line[2] == 'X':
                score1 += 7 #scissors vs rock (win)
                score2 += 2 #scissors vs paper (lose)
                continue
            elif line[2] == 'Y':
                score1 += 2 #scissors vs paper (lose)
                score2 += 6 #scissors vs scissors (tie)
                continue
            else:
                score1 += 6 #scissors vs scissors (tie)
                score2 += 7 #scissors vs rock (win)
                continue
        else:
            continue
