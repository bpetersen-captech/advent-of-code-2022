...
most recently received characters were all different. Specifically, it needs to report the number of characters from the beginning of the buffer to 
...
different. In this case, your subroutine should report the value 7, because the first start-of-packet marker is complete after 7 characters have 
been processed.
...
for letter in remaining:
    ...
    if (pos1 in pos2) or (pos1 in pos3) or (pos1 in pos4) or (pos2 in pos3) or (pos2 in pos4) or (pos3 in pos4):
        #print('match: ' + letter)
        inc+=1

    else:
        inc+=1
        print('no match, new letter is: ' + letter + ' at marker: ' + str(inc))        
        break
...
marker_count = 14 #length of string to read

first_fourteen = data[:marker_count] #first section of input
remaining = data[marker_count:] #remaining section of input to parse
result = first_fourteen #string to cycle through input
inc = marker_count #increment val to find position of unqiue string

for letter in remaining:
    inc+=1
    result = result[1:] + letter
    #print('inc: ' + str(inc) + ' result: ' + result)
